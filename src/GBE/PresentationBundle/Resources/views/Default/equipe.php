...
if(isset($_SESSION['email']))
{
	...
	if($perso_rang == 1)
	{
		...
		{
			echo '<h2 class="titre_choisi_troncon">Liste des &eacute;quipes et ses participants.</h2>';
			while($equipes = mysql_fetch_array($liste_equipes))
			{
				if($equipes['equipe'] != 'aucune')
				{
					...
					$liste_participant = mysql_query("SELECT id, prenom, nom, avatar, rang, page_collecte, equipe, page_equipe FROM users WHERE equipe='$nom_equipe' ORDER BY rang");
					echo '<ul>';
					while($liste_participants = mysql_fetch_array($liste_participant))
					{
						$id_personne = $liste_participants['id'];
						if((($liste_participants['page_collecte']!=null) and ($liste_participants['equipe']!='aucune') and ($liste_participants['page_equipe']!=null)) or ($liste_participants['rang'] == 1))
						{
							$style_supp = '';						
						}
						else
						...
						{
							echo '<a href="?t=Page_Profil&p='.$id_personne.'" class="lien_profil_administration"><li class="chef_equipe" '.$style_supp.'><strong>'.$liste_participants['prenom'].' '.$liste_participants['nom'].'</strong><img alt="'.$liste_participants['prenom'].' '.$liste_participants['nom'].'" src="'.$emplacement_avatar.'" title="'.$liste_participants['prenom'].' '.$liste_participants['nom'].'" class="photo_profil_administration_equipe"/></li></a>';		
						}
						else
						{
							echo '<a href="?t=Page_Profil&p='.$id_personne.'" class="lien_profil_administration"><li class="membre_equipe" '.$style_supp.'><strong>'.$liste_participants['prenom'].' '.$liste_participants['nom'].'</strong><img alt="'.$liste_participants['prenom'].' '.$liste_participants['nom'].'" src="'.$emplacement_avatar.'" title="'.$liste_participants['prenom'].' '.$liste_participants['nom'].'" class="photo_profil_administration_equipe"/></li></a>';	
						}
					}
					echo '</ul></div>';
				}
...
?>
		<h2 class="titre_choisi_troncon">Attention !</h2>	
		<p style="text-indent : 30px; ">Vous n'&ecirc;tes pas administrateur.</strong></p>
<?php
